...
function createComprehensivePrompt(
  ...
  return `
You are a medical AI assistant with access to authoritative medication databases. Given the following medication information:

${serialized}

${context ? `\nReference Data from French Government Database:\n${context}` : ''}
...
**IMPORTANT**: When French database reference data is available, prioritize using that information for suggestions, as it comes from an authoritative government source. Use this data to suggest:
- ATC codes (from French database)
- Composition details (substances, dosages, units)
